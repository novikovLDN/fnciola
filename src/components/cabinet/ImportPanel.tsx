...
import { IconImport, IconPlus } from '@/components/icons';
...
/** Панель импорта выписок — переиспользуется в «Счетах» и на отдельной странице. */
export function ImportPanel({ heading }: { heading?: string }) {
  ...
  function commit() {
    ...
    for (const r of toAdd) {
      addTx({ accountId: '', direction: r.direction, amountMinor: r.amountMinor, categoryKey: r.categoryKey, occurredAt: r.occurredAt, recurrence: 'one_time', note: r.description });
    }
  ...
  return (
    <div className="space-y-4">
      {heading && <h2 className="font-display text-lg font-bold">{heading}</h2>}
      <input ref={inputRef} type="file" accept={ACCEPTED_EXTENSIONS} onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFile(f); e.target.value = ''; }} className="hidden" />
      ...
      {(stage === 'idle' || stage === 'parsing') && (
        <div
          ...
          className={`card flex flex-col items-center gap-3 border-2 border-dashed py-12 text-center transition-colors ${dragOver ? 'border-accent bg-accent/10' : 'border-accent/30 bg-accent/5'}`}
        >
          ...
          {stage === 'parsing' ? (
            ...
            <>
              <p className="font-display text-lg font-bold">Перетащите выписку сюда</p>
      ...
      {stage === 'idle' && fatal && <p className="text-center text-sm text-negative">{fatal}</p>}
              ...
              <tbody>
                {rows.map((r, idx) => (
                  <tr key={r.key} className={`border-t border-ink/8 ${r.include ? '' : 'opacity-45'}`}>
                    ...
                    <td className="whitespace-nowrap px-2 py-2 tnum text-muted">{r.occurredAt}{r.isDup && <span className="ml-1 text-[10px]">дубль</span>}</td>
      ...
      <AnimatePresence>
        {stage === 'done' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card flex flex-col items-center gap-3 py-12 text-center">
            ...
            <button onClick={reset} className="btn btn-secondary mt-1">Импортировать ещё</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {stage === 'idle' && (
        <div className="card text-sm text-muted">
          ...
          <ul className="grid gap-1.5 sm:grid-cols-2">
            <li>• <b className="text-ink">CSV</b> — авто-определение разделителя и колонок</li>
